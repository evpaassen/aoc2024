package nl.erikvanpaassen.aoc2024.day7.b
...
data class Equation(val testValue: Long, val numbers: List<Long>) {
    ...
    fun canBeTrue(testValue: Long, numbers: List<Long>): Boolean {
        ...
        return testValue - lastNumber > 0 && canBeTrue(testValue - lastNumber, remainingNumbers)
                || testValue % lastNumber == 0L && canBeTrue(testValue / lastNumber, remainingNumbers)
                || testValue.toString().endsWith(lastNumber.toString()) && canBeTrue(testValue.toString().dropLast(lastNumber.toString().length).toLong(), remainingNumbers)
    }
